...
export default function Toast({ id, message, type = "success", onClose, duration = 5000 }) {
  ...
  return (
    ...
    >
      <div
        className={`${containerStyle} border shadow-lg rounded-xl px-4 py-3 min-w-75 max-w-md flex items-start gap-3`}
      >
        <span className={`${iconStyle} mt-0.5`} aria-hidden="true">
          {Icon}
        </span>
        <div className="flex-1 min-w-0">
          <p className="text-sm font-medium leading-relaxed">{message.replace(/_/g, " ")}</p>
        </div>
        <button
          onClick={dismiss}
